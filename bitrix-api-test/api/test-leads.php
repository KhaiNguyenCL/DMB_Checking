...
/**
 * Test Bitrix24 CRM Leads API
 * Get leads data from Bitrix24 CRM
 */
...
try {
    ...
    if ($action === 'fields') {
        ...
        $fields = \Bitrix\Crm\LeadTable::getMap();
        $fieldsData = [];
        ...
        $userFields = $GLOBALS['USER_FIELD_MANAGER']->GetUserFields('CRM_LEAD');
    ...
    // Get user fields for each lead
    foreach ($leads as &$lead) {
        $userFields = $GLOBALS['USER_FIELD_MANAGER']->GetUserFields('CRM_LEAD', $lead['ID']);
        foreach ($userFields as $field) {
            if (!empty($field['VALUE'])) {
                $lead[$field['FIELD_NAME']] = $field['VALUE'];
            }
        }
    }
