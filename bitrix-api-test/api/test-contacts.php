...
/**
 * Test Bitrix24 CRM Contacts API
 * Get contacts data from Bitrix24 CRM
 */
...
try {
    ...
    if ($action === 'fields') {
        ...
        $fields = \Bitrix\Crm\ContactTable::getMap();
        $fieldsData = [];
        ...
        $userFields = $GLOBALS['USER_FIELD_MANAGER']->GetUserFields('CRM_CONTACT');
    ...
    // Get user fields for each contact
    foreach ($contacts as &$contact) {
        $userFields = $GLOBALS['USER_FIELD_MANAGER']->GetUserFields('CRM_CONTACT', $contact['ID']);
        foreach ($userFields as $field) {
            if (!empty($field['VALUE'])) {
                $contact[$field['FIELD_NAME']] = $field['VALUE'];
            }
        }
    }
